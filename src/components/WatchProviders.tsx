      ...
      {providers.map((provider) => (
        <button
          ...
          className={twMerge(
            ...
            activeProviders.includes(provider.provider_id.toString()) ? "after:bg-transparent ring-2 ring-brand/50" : ""
          )}
          onClick={(e) => {
            handler(e, activeProviders);
            if (!activeProviders.includes(provider.provider_id.toString())) {
            ...
            setActiveProviders(
              activeProviders.filter(
                (providerId) => providerId !== provider.provider_id.toString()
              )
            );
          }}
        >
